...
public class DFS {
	...
	public boolean canChop( int id ){
		...
		boolean free = true;
		if(	boards.peekFirst().nodes[i+1][j].cont == Contained.BOX //if the box has a blocked tile down
			|| boards.peekFirst().nodes[i+1][j].cont == Contained.WALL 
			|| boards.peekFirst().nodes[i+1][j].cont == Contained.BOXNGOAL ){
			free = false;
			for( int k = 0; !hasGoal && k<boards.peekFirst().cols; k++ ){
				if( boards.peekFirst().nodes[i][k].cont == Contained.GOAL || boards.peekFirst().nodes[i+1][k].cont == Contained.FREESPACE )
					hasGoal = true;
			}
		}
		...
		if(	boards.peekFirst().nodes[i][j+1].cont == Contained.BOX //if the box has a blocked tile to the right
			...
			for( int k = 0; !hasGoal && k<boards.peekFirst().rows; k++ ){
				if( boards.peekFirst().nodes[k][j].cont == Contained.GOAL || boards.peekFirst().nodes[k][j+1].cont == Contained.FREESPACE )
					hasGoal = true;
			}
		}
